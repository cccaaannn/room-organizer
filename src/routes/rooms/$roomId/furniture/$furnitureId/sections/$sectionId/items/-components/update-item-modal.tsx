...
import { updateItem, UpdateItemScheme } from "@/routes/rooms/$roomId/furniture/$furnitureId/sections/$sectionId/items/-server-functions/update";
...
const UpdateItemModal = (props: Props) => {
	...
	const itemSelectValues = items.filter(r => r.id !== props.item.id).map(r => ({ value: r.id, label: r.name })) ?? [];
	...
	const form = useAppForm({
		defaultValues: {
			...
			tags: props.item.itemsTags.map(r => r.tagId),
			relatedItems: props.item.relatedItems.map(r => r.relatedItemId)
		},
		validators: {
			onChange: UpdateItemScheme
		},
		onSubmit: async ctx => {
			try {
				const response = await updateItem({ data: ctx.value });
